...
export const AdminDashboard: React.FC = () => {
  ...
  // Fetch admin summary (synchronizes with Firestore users automatically)
  const fetchEmployees = async () => {
    if (!isAdmin || !currentUser) return;
    setLoading(true);
    try {
      // 1. In parallel or before query, sync any existing users from Firestore collection
      try {
        ...
        console.warn('Firestore direct user sync notice:', fsErr);
      }
      ...
      if (res.ok) {
        ...
        const deduped = rawList.filter((item: any) => {
          const key = (item.user?.email || item.user?.id || '').toLowerCase().trim();
  ...
  const inspectEmployee = async (employeeId: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/employee/${employeeId}/attendance`, {
        headers: { 'x-user-role': currentUser.role },
      });
      if (res.ok) {
        const data = await res.json();
  ...
  const handleAddEmployee = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newEmpName || !newEmpEmail || !currentUser) return;
    setIsAdding(true);
    try {
      ...
      if (res.ok) {
        ...
        if (data.user) {
          ...
          try {
            ...
            console.warn('Firestore employee write notice:', fsErr);
          }
        }
  ...
  // Test Mode: Reset all attendance records for today
  ...
  const handleResetAllAttendance = async () => {
    ...
    try {
      const res = await fetch('/api/admin/reset-attendance', {
        ...
        body: JSON.stringify({ date: '2026-09-04' }),
      });
      if (res.ok) {
        setTestActionMsg('All attendance sessions have been reset to Not Clocked In.');
        await fetchEmployees();
        if (selectedEmployeeId) {
          await inspectEmployee(selectedEmployeeId);
        }
      }
    } catch (err) {
      ...
      setTimeout(() => setTestActionMsg(null), 5000);
    }
  };

  // Test Mode: Reset specific employee's attendance for today
  const handleResetSingleEmployee = async (empId: string) => {
    if (!currentUser) return;
    try {
      const res = await fetch('/api/attendance/reset', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: empId, date: '2026-09-04' }),
      });
      if (res.ok) {
        setTestActionMsg('Shift reset to Not Clocked In for this member.');
        await fetchEmployees();
        await inspectEmployee(empId);
      }
    } catch (err) {
      console.error('Failed to reset employee attendance:', err);
    } finally {
      setTimeout(() => setTestActionMsg(null), 5000);
    }
  };

  // Test Mode: Simulate complete shift for testing inspection KPIs
  const handleSimulateEmployeeShift = async (empId: string, isSunday = false) => {
    if (!currentUser) return;
    try {
      const targetDate = isSunday ? '2026-09-06' : '2026-09-04';
      const res = await fetch('/api/admin/simulate-shift', {
        ...
        body: JSON.stringify({
          ...
          date: targetDate,
          hours: 8.5,
          isSundayShift: isSunday,
        }),
      });
      if (res.ok) {
        setTestActionMsg(`Simulated 8.5h ${isSunday ? 'Sunday (+1 Credit)' : 'standard'} shift logged! Check inspection KPIs.`);
        await fetchEmployees();
        await inspectEmployee(empId);
      }
    } catch (err) {
      console.error('Failed to simulate shift:', err);
    } finally {
      setTimeout(() => setTestActionMsg(null), 5000);
    }
  };
      ...
      <div className="max-w-2xl mx-auto px-4 py-16 text-center animate-in fade-in duration-300">
        <div className="rounded-3xl bg-[#0F172A]/80 p-8 sm:p-12 border border-red-500/30 shadow-2xl relative overflow-hidden backdrop-blur-xl">
          ...
          <p className="text-slate-400 max-w-md mx-auto mt-3 text-sm leading-relaxed">
            The Master Corporate Attendance Dashboard is reserved strictly for authenticated system administrators. Your account ({currentUser?.email}) does not have administrative privileges.
          </p>
        </div>
      </div>
    );
  ...
  const filteredEmployees = employees.filter((item) => {
    const dept = item.user.department || 'General';
    const matchesSearch =
  ...
  const dynamicDepts = Array.from(new Set(employees.map(e => e.user.department || 'General').filter(Boolean)));
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="relative overflow-hidden rounded-3xl p-6 sm:p-8 bg-[#0F172A]/80 backdrop-blur-xl border border-purple-500/20 shadow-xl">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">
              Real-time employee clock status, automated leave balance audit, and yellow alert oversight.
            </p>
          </div>

          <div className="flex flex-wrap items-center gap-2.5 self-start sm:self-auto">
            <button
              id="admin-reset-all-attendance-btn"
              type="button"
              onClick={handleResetAllAttendance}
              disabled={resettingAll}
              title="Reset all attendance sessions for today back to Not Clocked In (for testing check in & check out)"
              className="flex items-center space-x-1.5 px-3 py-2 rounded-xl bg-red-950/40 hover:bg-red-900/60 text-red-300 font-medium text-xs border border-red-500/30 transition-all cursor-pointer"
            >
              <RotateCcw className={`w-3.5 h-3.5 ${resettingAll ? 'animate-spin' : ''}`} />
              <span>Reset All (Test Mode)</span>
            </button>
            ...
            <div className="px-3.5 py-1.5 rounded-xl bg-slate-900/80 border border-white/10 text-right">
              <span className="text-[10px] text-slate-400 uppercase tracking-wider block">Staff Roster</span>
              <span className="text-base font-bold text-white font-mono">{employees.length} Members</span>
            </div>
          </div>
        </div>
        ...
        {testActionMsg && (
          <div className="mt-3 p-3 rounded-xl bg-emerald-950/60 border border-emerald-500/40 text-xs text-emerald-200 flex items-center space-x-2 animate-in fade-in duration-200">
            <Check className="w-4 h-4 text-emerald-400 shrink-0" />
            <span>{testActionMsg}</span>
          </div>
        )}
      </div>
      ...
      <div className="rounded-3xl bg-[#0F172A]/80 backdrop-blur-xl border border-white/10 overflow-hidden shadow-2xl">
        {loading ? (
          <div className="py-16 text-center text-slate-400 flex flex-col items-center justify-center">
            ...
            <span className="text-xs">Loading employee attendance records...</span>
          </div>
          ...
          <div className="py-16 px-6 text-center text-slate-400 flex flex-col items-center justify-center">
            ...
            <p className="text-xs text-slate-400 max-w-sm mb-4">
              Registered employees will automatically appear here once authenticated, or you can register new team members manually.
            </p>
            <button
              onClick={() => setShowAddModal(true)}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-white/10 bg-slate-900/70 text-[11px] font-semibold uppercase tracking-wider text-slate-400">
                  ...
                  <th className="py-4 px-4">Month Hours</th>
                  <th className="py-4 px-4">Leaves Taken (Orange/Red)</th>
                  <th className="py-4 px-4">Incomplete Logs</th>
                  <th className="py-4 px-4">Today Status</th>
                  <th className="py-4 px-6 text-right">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5 text-sm">
                {filteredEmployees.map((emp) => {
                  const isClockedInToday = emp.todayStatus === 'CHECKED_IN';
                  const isCompletedToday = emp.todayStatus === 'PRESENT';

                  return (
                    <tr
                      key={emp.user.id}
                      className="hover:bg-slate-800/40 transition-colors group cursor-pointer"
                      onClick={() => inspectEmployee(emp.user.id)}
                    >
                      {/* Name & Code */}
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-slate-400 flex items-center space-x-1.5 mt-0.5">
                              <span className="font-mono text-[11px] text-slate-400">{emp.user.employeeCode}</span>
                              <span>•</span>
                      ...
                      <td className="py-4 px-4">
                        <div className="text-xs font-medium text-slate-300">
                          {emp.user.department || 'General'}
                        </div>
                        <div className="text-[11px] text-slate-400">
                          {emp.user.jobTitle || (emp.user.role === 'ADMIN' ? 'System Administrator' : 'Staff Member')}
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-4">
                        <div className="font-mono font-semibold text-white">
                          {emp.stats.totalHoursWorked}h
                        </div>
                        <div className="text-[11px] text-slate-400">
                          Avg: {emp.stats.averageHoursLoggedIn || '0h'}/shift
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-4">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-semibold bg-orange-500/20 text-orange-300 border border-orange-500/30">
                            {emp.stats.paidLeavesUsed} Paid
                          </span>

                          {/* Unpaid Leaves (Red) */}
                          {emp.stats.unpaidLeaves > 0 ? (
                            <span className="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-semibold bg-red-500/20 text-red-300 border border-red-500/30">
                              {emp.stats.unpaidLeaves} Unpaid
                            </span>
                          ) : (
                            <span className="text-[11px] text-slate-400">0 Unpaid</span>
                          )}

                          {/* Sunday credit bonus */}
                          {emp.stats.sundayCompensationsEarned > 0 && (
                            <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold bg-fuchsia-500/20 text-fuchsia-300 border border-fuchsia-500/40">
                              +{emp.stats.sundayCompensationsEarned} Sun
                            </span>
                          )}
                        </div>
                      </td>

                      {/* Incomplete Logs (Yellow) */}
                      <td className="py-4 px-4">
                        {emp.stats.incompleteCheckoutsCount > 0 ? (
                          <span className="inline-flex items-center space-x-1 px-2.5 py-0.5 rounded-full text-xs font-bold bg-amber-500/20 text-amber-300 border border-amber-500/40">
                            ...
                            <span>{emp.stats.incompleteCheckoutsCount} Missed 11:59PM</span>
                          </span>
                        ) : (
                          <span className="text-xs text-slate-400">Clean (0)</span>
                        )}
                      </td>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="w-full max-w-md bg-[#0F172A] border border-white/10 rounded-2xl p-6 shadow-2xl relative">
            ...
            <h3 className="text-lg font-semibold text-white mb-1">Add Team Member</h3>
            <p className="text-xs text-slate-400 mb-4">
              Register a corporate team member to track monthly attendance and shift compensation.
            </p>
            ...
            <form onSubmit={handleAddEmployee} className="space-y-3.5">
              ...
              <div className="pt-2 flex justify-end gap-2">
                ...
                >
                  {isAdding ? 'Adding...' : 'Register Employee'}
                </button>
              </div>
            </form>
          </div>
      ...
      {/* Slide-over Inspection Panel for Detailed Employee Calendar */}
            ...
            <div className="w-screen max-w-4xl bg-[#0F172A] border-l border-white/10 shadow-2xl p-6 sm:p-8 overflow-y-auto">
              ...
              <div className="flex items-center justify-between pb-6 border-b border-white/10 mb-6">
                <div className="flex items-center space-x-4">
                  <img
                    src={employeeDetails.employee.avatarUrl}
                    alt={employeeDetails.employee.name}
                  ...
                  <div>
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="px-2 py-0.5 rounded text-[11px] font-mono bg-purple-500/20 text-purple-300">
                        {employeeDetails.employee.employeeCode}
                      </span>
                    </div>
                    <p className="text-xs text-slate-400">
                      {employeeDetails.employee.jobTitle} • {employeeDetails.employee.department}
                    </p>
                  </div>
                </div>
              ...
              {/* Test Mode / Shift Simulation Bar */}
              <div className="mb-6 p-4 rounded-2xl bg-slate-900/90 border border-white/10 flex flex-wrap items-center justify-between gap-3">
                <div>
                  <span className="text-xs font-bold text-slate-200 uppercase tracking-wider block">
                    Shift Testing & Simulation
                  </span>
                  <span className="text-[11px] text-slate-400">
                    Test attendance flow, verify live KPI calculation, or reset shift state.
                  </span>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <button
                    type="button"
                    onClick={() => handleSimulateEmployeeShift(employeeDetails.employee.id, false)}
                    className="flex items-center space-x-1.5 px-3 py-1.5 rounded-xl bg-blue-600/20 hover:bg-blue-600/30 text-blue-300 border border-blue-500/30 text-xs font-semibold transition-all cursor-pointer"
                    title="Simulate a completed 8.5-hour weekday shift on today"
                  >
                    <PlayCircle className="w-3.5 h-3.5 text-blue-400" />
                    <span>Simulate 8.5h Shift</span>
                  </button>

                  <button
                    type="button"
                    onClick={() => handleSimulateEmployeeShift(employeeDetails.employee.id, true)}
                    className="flex items-center space-x-1.5 px-3 py-1.5 rounded-xl bg-purple-600/20 hover:bg-purple-600/30 text-fuchsia-300 border border-purple-500/30 text-xs font-semibold transition-all cursor-pointer"
                    title="Simulate a Sunday shift to test +1 Paid Holiday Credit award"
                  >
                    <Sparkles className="w-3.5 h-3.5 text-fuchsia-400" />
                    <span>Simulate Sunday Shift (+1 Credit)</span>
                  </button>

                  <button
                    type="button"
                    onClick={() => handleResetSingleEmployee(employeeDetails.employee.id)}
                    className="flex items-center space-x-1.5 px-3 py-1.5 rounded-xl bg-red-600/20 hover:bg-red-600/30 text-red-300 border border-red-500/30 text-xs font-semibold transition-all cursor-pointer"
                    title="Reset today's shift back to Not Clocked In"
                  >
                    <RotateCcw className="w-3.5 h-3.5 text-red-400" />
                    <span>Reset to Not Clocked In</span>
                  </button>
                </div>
              </div>
              ...
              <StatsRow
                stats={employeeDetails.evaluation.stats}
                monthName="September 2026"
              />

              {/* Sunday Shift Compensation Audit Trail Box */}
              {employeeDetails.evaluation.leaveAudits.length > 0 && (
                <div className="mb-6 p-4 rounded-2xl bg-purple-950/30 border border-purple-500/30">
                  <div className="flex items-center space-x-2 text-xs font-bold text-fuchsia-300 uppercase tracking-wider mb-2">
                    ...
                    <span>Sunday Shift Compensation Audit Trail</span>
                  </div>
                  <div className="space-y-2">
                    {employeeDetails.evaluation.leaveAudits.map((audit) => (
                      <div
                        key={audit.id}
                      ...
                      >
                        <div>
                          <div className="font-semibold text-white">{audit.reason}</div>
                          <div className="text-[11px] text-fuchsia-300/80 mt-0.5">
                            Credit: +{audit.creditAmount} Paid Holiday • Neutralizes leave threshold
                          </div>
                        </div>
                        {audit.appliedToLeaveDate && (
                          <span className="px-2.5 py-1 rounded-lg bg-orange-500/20 text-orange-300 border border-orange-500/30 text-[11px] font-semibold">
                            Applied to {audit.appliedToLeaveDate}
                          </span>
                        )}
                      </div>
                    ))}
              ...
              <MonthlyCalendarGrid
                days={employeeDetails.evaluation.days}
              ...
              {/* Day Inspector Modal if clicked from inside slide-over */}
              {inspectDay && (
                <DayDetailModal
                  day={inspectDay}
